...
// 전역적인(public) 변수를 선언하고자 할 때 사용
// 요즘은 많이 쓰이지 않음 const, let 사용을 권장
var 변수1 = 100;
...
// 한 번 값이 저장되면 값을 변경할 수 없다
// java final과 같다
// primitive(int, float, string)이 아닌
// 객체타입의 변수들을 만들 때는 가급적 const를 사용하라
// 함수를 선언할때도 사용
const 상수1 = 300;
 ...
 * Java로 예를 들면
 * 반드시 클래스(interface...)를 만드로
 * 클래스 내부에서 함수를 선언하여 호출하는 프로그래밍구조
 *
 * method는 호출을 하려면 먼저 객체를 생성하고
 * 1. 클래스를 객체로 선언 생성하고
 * 2. 생성된 객체를 통해서 호출을 하는 구조
 * Class class = new Class();
 * Class.method(); 와 같은 형식
 *
 * method는 다음과 같은 코드를 만나면
 * 변수 = class.method()
 * class 객체에 포함된 method()를 실행하여 그 결과를 변수에담아라
 *
 *
 */

// 함수의 결과를 변수 "함수1"에 담으라는 것이 아니라
